...
| PowerShell and cmd | Partial | PowerShell wrapper injects bounded `Get-Command -All` alias/function/cmdlet evidence; paired contract tests cover selected and losing external candidates | Real cmd ancestry and current-session builtin injection remain unverified |
| bash | Verified | bash wrapper injects bounded alias/function/builtin evidence; same Documents checkout was exercised from Ubuntu-24.04 WSL | Validation covers Git Bash on Windows and one WSL distribution |
...
| Git, Node, and npm resolution | Verified on Windows and WSL | Four selection states, selected binding kind, candidate count, Windows/Linux origin, PE/ELF/script format, reason, and independent verification command are recorded in JSON and Markdown |
...
| `TOOL001` | Implemented | Complete shell evidence warns when npm is selected but Node is not found; uncertain Node candidates do not trigger it |
...
| JSON output | Implemented | Schema `0.6.0` adds four-state selection, selected binding kind, reason, verification command, and finding reverification steps; it is not frozen yet |
