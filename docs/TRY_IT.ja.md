...
この手順は、完成済みのRust CLIとローカルGUIを自分の環境で動かし、「表示が正しいか」「判断に役立つか」を確認するためのものです。個人情報を公開せずに試せるよう、補助スクリプトは常に自動匿名化Markdownを`target/user-validation/`へ保存します。このディレクトリはGitの管理対象外です。

> 現在はプレアルファ版です。配布バイナリはまだないため、ソースからビルドします。Rust、依存ライブラリ、Microsoft Build Toolsはいずれも無料で利用できます。初回ビルド時は無料のOSS依存ライブラリを取得するためネットワークを使いますが、ExecLocus本体の通常診断は外部ネットワークへ接続しません。GUIはPC内のloopback通信だけを使用します。
...
| 確認対象 | できること | 実データ |
|---|---|---|
| [静的な操作デモ](demo/prototype/index.html) | インストール前に画面構成と操作感を確認 | `file://`または通常の静的配信では合成データ |
| ローカルGUI | ボタン1つでCLIと同じ読み取り専用診断を実行し、候補・影響・推奨対応・再確認コマンドを表示 | 自分のローカル環境 |
| [Windowsデスクトップ開発版](DESKTOP_TRY_IT.ja.md) | 同じGUIをブラウザ枠なしの署名なしWindowsアプリで試す | 自分のローカル環境 |
| Rust CLI | Windows／WSL、ディストリビューション、利用者、シェル、候補実行ファイル、ファイルシステム境界を観測 | 自分のローカル環境 |

静的な操作デモは診断を実行しません。実用性の判断には、以下のローカルGUIまたはCLI確認を行ってください。
...
GUIは外部サービスではなく、ExecLocusが自分のPC内の`127.0.0.1`だけで一時起動する画面です。診断内容のアップロード、テレメトリ、有料API呼び出しはありません。ターミナルを閉じるか`Ctrl+C`を押すと終了します。
...
ブラウザが自動で開いたら、次の順で確認します。

1. `診断を実行`を押す。
2. `診断`でWindows／WSL、ディストリビューション、シェル、プロジェクト位置を確認する。
3. `比較`でGit、Node、npm、Codex、Claude Codeの4状態、候補数、由来、形式、選択理由、確認コマンドを確認する。
4. `説明`で影響、推奨対応、再確認手順と根拠を確認する。
5. 右上の`EN`／`日本語`で、実診断結果を含めて言語が切り替わることを確認する。
6. `共有`で自動匿名化されたMarkdownだけをコピーする。

`診断`、`比較`、`説明`は手元で判断するための画面で、利用者名やローカル絶対パスを表示する場合があります。スクリーンショットや内容を公開しないでください。公開候補にできるのは`共有`画面の内容だけです。それも公開前に自分の目で再確認してください。

## 2. Windows PowerShellでCLIを確認
