...
- [直接の代替手段・現在の回避策](ALTERNATIVES.md)
...
日本語では「AIコーディングエージェントの現在のコンテキストが、どの実行ファイルを選ぶか、なぜその結果になるかを証拠付きで説明する」が中核となる。エージェントが過去に実行した対象を示すのは、processまたはinvocationの証拠がある場合に限る。
...
したがって、競合不在を市場不在と決めつける必要はない。ただし、隣接市場のスター数だけではExecLocus固有の需要は証明できない。重い機能開発の前に、10人以上の協力者・10環境以上で診断し、少なくとも3件で「確認可能な結論または判断につながり、従来の手作業より役に立った」という結果を需要検証ゲートとする。差異の発見だけでは需要の証明に数えない。
...
- これは競合の完全列挙ではない
- 最初の50候補一覧、検索query、除外理由の完全なlogは保存されていないため、選定過程は第三者が同じ結果へ再現できない
- 下記の定量値は取得時点のsnapshot、7項目はその観測からExecLocus向けに編集した仮説であり、因果関係の証明ではない
...
ExecLocusも「環境情報を表示するCLI」では弱い。「Codex／Claude Codeの現在のコンテキストで、Windows版とWSL版のどちらが選ばれるかを説明するCLI」と表現する。
...
- 10人以上・10環境以上で試し、3件以上の確認済みで役に立った結論を得る
