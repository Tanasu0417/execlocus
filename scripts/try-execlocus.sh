...
cargo run --quiet --locked -- --profile "$profile" report --format markdown >"$report_path"
report_exit_code=$?
if (( report_exit_code >= 2 )); then
  printf 'ExecLocus could not create the report (exit code %s).\n' "$report_exit_code" >&2
  exit "$report_exit_code"
fi
cargo run --quiet --locked -- --profile "$profile" report --format json --redact >"$json_path"
json_exit_code=$?
...
if [[ "${SHOW_LOCAL_DETAILS:-0}" == "1" ]]; then
  ...
  cargo run --quiet --locked -- --profile "$profile" check || true
  cargo run --quiet --locked -- --profile "$profile" explain FS001
fi
